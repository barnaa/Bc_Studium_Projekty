...
**/
  // Inicializujeme ukazatele, aby neukazovali niekam do pamate
  L->Act = NULL;
  L->First = NULL;
  return;
}
...
***/
  // Odstranujeme prvky, kym nenarazime na koniec zoznamu
	while(L->First != NULL){
    L->Act = L->First->ptr;
    free(L->First);
    L->First = L->Act;
...
** volá funkci Error().
**/
  // Zazalohujeme si ukazatel na prvy prvok zoznamu do pomocnej premennej,
  // a alokujeme pamat, potom prepojime zoznam
  tElemPtr tmp = L->First;
  if((L->First = malloc(sizeof(struct tElem))) == NULL)
    Error();
  else{
    L->First->ptr = tmp;
    L->First->data = val;
...
**/
  if(L->First != NULL){
    if (L->First == L->Act)
      L->Act = NULL;
    // Zalohujeme si ukazatel na prvok za prvym prvkom zoznamu, uvolnime prvy
    // prvok a nastavime ukazatel z pomocnej premennej ako prvy
...
**/
    ...
    // Ulozime si ukazatel na prvok za nasledujucim prvkom, odstranime dalsi
    // prvok a prepojime zoznam
...
** zavolá funkci Error().
**/
  // Ulozime si ukazatel na nasledujuci prvok do pomocnej
  // premennej, alokujeme pamat a prepojime zoznam
  if(L->Act != NULL){
    tElemPtr tmp = L->Act->ptr;
    if((L->Act->ptr = malloc(sizeof(struct tElem))) == NULL)
      Error();
    else{
      L->Act->ptr->ptr = tmp;
      L->Act->ptr->data = val;
